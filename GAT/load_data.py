...
from utils import sparse_diag, sparse_select
...
class Collate():
    """ Collate function for mini-batch, can't use default collate_fn due to batch in different size"""
    def __init__(self, adj):
        self.adj = adj
        self.edge = self.adj._indices()

    def __call__(self, batch):
        idx = torch.LongTensor([i for i in batch])
        adj = sparse_select(self.adj, 0, idx)
        neighbor = torch.sparse.sum(adj, dim=0)._indices()[0]
        edge = self.edge[:, (self.edge[0].view(-1, 1) == neighbor).any(-1)]
